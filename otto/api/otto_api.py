...
class OttoApi:
    ...
    def __init__(self):
        ...
        print(f"Secret Key: {self._app.config['SECRET_KEY']}")
        self._create_routes()
    ...
    def _create_routes(self):
        ...
        def app_login():
            ...
            if not login_request or 'application-name' not in login_request or 'password' not in login_request:
                return jsonify({'message': 'Username AND Password are required for network application authentication'}), 403
            ...
            cursor.execute(
                'SELECT * FROM network_applications where app_name = %s AND password = %s', (login_request['application-name'], login_request['password'])
            )
            ...
            if cursor.fetchone() is not None:
              ...
              return jsonify({'message': f"Incorrect password for application {login_request['application-name']}"}), 403
