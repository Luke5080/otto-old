...
def main():
    ...
    main_arg_parser = argparse.ArgumentParser(prog="Otto",description="Intent Based Northbound Interface for SDN controllers")

    main_arg_parser.add_argument('--controller', required=True, help="Controller to be used with Otto - Can either be Ryu or ONOS")
    ...
    main_arg_parser.add_argument('--api-pool-size', required=False, help="IntentProcessor Pool Size to use for Otto REST APIs")
    main_arg_parser.add_argument('--pool-models', required=False, help="Models to be used in REST API IntentProcessor Pool")
    ...
    if args.api:
       ...
       logger.info(f"Creating IntentProcessor Pool with a size of {otto_flask_api.intent_processor_pool.pool_size} with models {otto_flask_api.intent_processor_pool.models}")
